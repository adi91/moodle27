...
This document describes how to copy and customise the powerup (mghccbbase) theme so that
...
From your Moodle theme directory right click on powerup and then copy and paste back
into your Moodle theme directory. You should now have a folder called Copy of powerup.
...
underscores. For the purpose of this tutorial we will call the theme 'poweruptheme'.

On opening 'poweruptheme' your you will find several files and sub-directories which have
files within them.
...
where the old theme name occurs, in this case powerup. So using the above list as
a guide, search through and change all the instances of the theme name
'powerup' to 'poweruptheme'. This includes the filename of the lang/en/theme_powerup.php.
You need to change this to 'theme_poweruptheme.php'.
...
and then click on (poweruptheme) or whatever you renamed your theme to,
from the list of theme names that appear at this point in the side block.

Customisation using custom theme settings
-----------------------------------------

The settings page for the powerup theme can be located by navigating to:

Administration > Site Administration > Appearance > Themes > powerup
